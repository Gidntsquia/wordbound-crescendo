...
// Regenerates the analyzed fields of src/recordings/recordedFurElise.json
// from public/audio/fur-elise.mp3.
//
...
//
// The output JSON's id/title/composer/performer/audio/excerpt fields are
// written once by tools/fetch-audio.js from tools/audio-manifest.json and
// left untouched here; this only overwrites durationSec/peak/loudness/
// dynamics. So adding a recording means running fetch-audio.js first (it
// writes the initial JSON from the manifest) and then this to fill in the
// analysis.
//
//   node tools/analyze-audio-piece.js [--in public/audio/fur-elise.mp3]
//                                     [--out src/recordings/recordedFurElise.json]
const fs = require('fs');
...
const OUT = path.resolve(
  ROOT,
  arg('out', 'src/recordings/recordedFurElise.json'),
);
const PORT = 9893;
...
(async () => {
  ...
  if (!fs.existsSync(OUT)) {
    console.error(
      'no such file: ' +
        OUT +
        '\n  This tool fills in an existing piece JSON file, it does not create one.' +
        '\n  Run tools/fetch-audio.js first -- it writes the initial JSON' +
        '\n  (id/title/composer/performer/audio) from the manifest.',
    );
    process.exit(1);
  }
  const existing = JSON.parse(fs.readFileSync(OUT, 'utf8'));
  // Recordings arrive at wildly different levels -- the Fur Elise track is
  // normalised to full scale, the Moonlight one peaks at about a tenth of it.
  // Playback level-matches on `loudness` (the 95th percentile of the raw RMS
  // envelope, which tracks how loud a piece SOUNDS) and uses `peak` only to
  ...
  const updated = {
    ...
    loudness,
    dynamics: {
      keyframes: keep.map(([s, i]) => ({ sec: s, intensity: i })),
      surges: surges.map((s) => ({
        sec: s.sec,
