          ...
          <p className="sb-hint sb-title-best">
            {best.word ? (
              <>
                ...
                {best.runs === 1 ? '' : 's'}
                {best.winsByKey && best.winsByKey[key] ? (
                  <>
                    {' '}
                    ({best.winsByKey[key]} in {SB.KEY_DEFS[key].name})
                  </>
                ) : (
                  ''
                )}
        ...
        (phase === 'live' || phase === 'scoring' || phase === 'won') && (
          <section className="sb-play" ref={playRef}>
            ...
            {inking && (
              <div className="sb-inking">
                ...
                <button
                  ...
                  disabled={
                    !inking.ids.length ||
                    (inking.ink.needsVowel && !inking.vowel)
                  }
                >
            ...
            {formable && barredNow.length > 0 && (
              <p className="sb-hint sb-warn-line">
                {barredNow.map((t) => t.letter).join(', ')} has been played this
                round — {round.rule.name}.
              </p>
            )}
          </section>
        )}
