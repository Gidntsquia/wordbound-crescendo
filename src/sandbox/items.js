...
(function () {
  ...
  var MUSIC = {};
  MUSIC_WORDS.forEach(function (w) {
    MUSIC[w] = 1;
  });
  Sandbox.isMusicWord = function (word) {
  ...
  var BARD = {};
  BARD_WORDS.forEach(function (w) {
    BARD[w] = 1;
  });
  Sandbox.isBardWord = function (word) {
  ...
  Sandbox.ITEMS = [
    ...
    },
    {
      ...
      score: function (c, a) {
        var n = c.run ? c.run.itemState.refrain || 0 : 0;
        if (!n) return null;
        a.mult += n;
        return '+' + n + ' mult';
      },
      onPlayed: function (run) {
    ...
    },
    {
      ...
      score: function (c, a) {
        if (!Sandbox.isMusicWord(c.word)) return null;
        a.mult *= 2;
    ...
    },
    {
      ...
      score: function (c, a) {
        if (!Sandbox.isBardWord(c.word)) return null;
        a.mult *= 2;
    ...
    // too (Sandbox.CRESCENDO's window in audioPiece.js, run.extendCrescendo).
    {
      ...
      onPlayed: function (run, breakdown) {
    ...
    // its ×held mult.
    {
      ...
      score: function (c, a) {
        var held = (c.held || []).filter(function (t) {
          return t && t.letter;
        });
        ...
        a.chord = {
          word: found[0].word,
          points: Sandbox.chordPoints(found[0].word, c.tune),
        };
        return null;
      },
    },
  ...
  Sandbox.ITEM_DEFS = {};
  Sandbox.ITEMS.forEach(function (it) {
    Sandbox.ITEM_DEFS[it.id] = it;
  });
  ...
  Sandbox.applyItems = function (ctx, acc) {
    ...
    (ctx.items || []).forEach(function (id) {
      ...
      if (note)
        notes.push(
          Sandbox.describeDelta(
            { id: id, name: it.name, note: note },
            p0,
            m0,
            acc,
          ),
  ...
  Sandbox.describeDelta = function (note, p0, m0, acc) {
    ...
    var additive =
      Math.abs(ratio - 1) < 1e-9 || note.note.indexOf('\u00d7') < 0;
