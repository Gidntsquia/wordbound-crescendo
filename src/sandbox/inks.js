...
(function () {
  ...
  Sandbox.INKS = [
    ...
    },
    {
      ...
      name: 'Coin',
      targets: 0,
      hint: 'Double your gold, up to +10',
    },
  ];
  Sandbox.INK_DEFS = {};
  Sandbox.INKS.forEach(function (ink) {
    Sandbox.INK_DEFS[ink.id] = ink;
  });
  ...
  Sandbox.applyInk = function (run, inkId, tileIds, extra) {
    ...
    var tiles = ids
      .map(function (id) {
        return run.deck.find(function (t) {
          return t.id === id;
        });
      })
      .filter(Boolean);
    ...
    if (ink.id === 'vowel') {
      ...
      if (Sandbox.VOWELS.indexOf(v) < 0)
        return { ok: false, reason: 'Choose a vowel.' };
      tiles.forEach(function (t) {
        t.letter = v;
