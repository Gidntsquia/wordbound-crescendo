# Progress archive — Wordbound: Crescendo

Entries moved VERBATIM from PROGRESS.md by rotation (first rotation 2026-08-24, per
GOALS.md's STATE HYGIENE rule). Never edit entries; newer rotations append below.
...
Same repo-hygiene fix needed again at start (stale local `main`/`origin/main`
refs, `HEAD` detached ~50 commits ahead) -- resolved the same way. SHAKESPEARE
GUIDE + AUTHOR SHOPKEEPERS's only remaining scope (author exclusive items) is
explicitly blocked on the ITEMS ticket per its own last several notes, so per
GOALS.md's "blocked -> move to the next item" rule, picked up ITEMS next
(large, self-contained, next unchecked item after that).

**What was done:** implemented 2 of Jaxon's 4 signature items --
**RITARDANDO** (slows all enemy music via a global tempo-scale multiplier)
and **POETIC LICENSE** (any exactly-3-letter rack combination counts as a
playable word). Deliberately did NOT attempt FORTISSIMO (rack-capacity
halving + double-size tile rendering, a real CSS/mobile-layout change) or
THE INVERTED SCORE (a flip-mapping + reversed-letter-order dictionary check)
this run -- both need their own separate engine/rendering work, not a small
addition alongside these two, and doing all 4 shallowly in one run risked
skipping the ticket's own "sim-check this" bar for the trickier two.

- **Ritardando**: `js/wordbound/items.js` got `statMods.tempoScale: 0.75`
  and `Items.getTempoScale(player)` (multiplies together any owned
  tempoScale statMods; 1 = no-op, mirrors `getRackCapacity`'s shape).
  `js/wordbound/game.js`'s new `computeDuelTempoScale()` combines this with
  the pre-existing Largo accessibility assist's own scale
  MULTIPLICATIVELY (0.6 Largo * 0.75 item = 0.45 combined) -- used at both
  `Game.startDuelFight` (fight-start) and `Game.setLargoEnabled` (so
  toggling Largo mid-duel while the item is owned recombines correctly
  instead of resetting to 1 or 0.6 flatly), exposed as
  `Game._computeDuelTempoScale` for jsdom-safe unit testing since it's pure
  (no AudioContext touched, unlike actually starting a duel fight -- the
  same hard-jsdom-crash hazard this file has documented many times before).
  0.75 is deliberately a SMALLER effect than Largo alone -- a purchasable
  build item shouldn't just replicate an accessibility toggle's full power.
- **Poetic License**: added a second validity gate inside
  `js/wordbound/combat.js`'s `playWord`/`previewWord` -- confirmed by grep
  this is the ONE choke point both vanilla and React share (React's
  CombatScreen only ever calls `Combat.previewWord`, which calls
  `playWord` internally). New `Items.bypassesWordValidity(word, player)`:
  an exactly-3-letter combination formable from the rack is playable even
  when `Lexicon.isValidWord` rejects it as not a real word. Scoring is
  completely untouched -- `scoreWord` has no idea whether the letters
  ...
  ticket's own "keep base scoring low so it's a floor-raiser" requirement
  fall out of the EXISTING formula for free, no special case needed:
  `lengthBonus` only starts past length 4 and `bingoBonus` needs the whole
  rack, so a 3-letter play (real or bypassed) is already this engine's
  lowest-scoring shape. Sim-checked the literal worst case by hand rather
  than building a new statistical simulator for one item: Q+Z+X, the
  letter pool's 3 highest-value letters (10/10/8), scores 28 raw with zero
  length/bingo bonus -- comparable to a single mediocre 5-letter real word,
  and drawing all three of those exact tiles in one 7-tile rack is already
  rare under `LETTER_POOL`'s weights (1 copy of each in the whole pool).
  Not a degenerate optimum; a guaranteed-but-modest floor action for an
  otherwise dead rack, which is the ticket's stated intent. A small
  `onWordPlayed` hook (feedback only, no damage change) announces the
  bypass with a message, but only when one was actually exercised (a real
  3-letter word needs no license and stays silent) -- matches this file's
  existing "silent modifiers don't create builds" convention.

**A real judgment call, documented rather than silently resolved:** neither
item has a shop/treasure roll test targeting it BY NAME yet (same gap every
prior item batch in this file already carries -- items only reach players
through the existing roll pools they're registered in via `ITEM_DEFS`, and
both new ones are). Not a regression, just not a new guarantee either.

**Verified:**
- `npm test` (jsdom dom-check): ALL CHECKS PASSED, +15 new checks -- Poetic
  License unplayable without the item via BOTH `playWord` and `previewWord`
  (confirming they agree, not a duplicated formula), playable and correctly
  scored (28 for the QZX worst case) with it, the proc-message firing only
  on a genuine bypass (a real word like CAT logs nothing), still respecting
  rack formability (a combo the rack can't form still fails) and the
  exactly-3-letters restriction (a 4-letter non-word stays rejected);
  `Items.getTempoScale` in isolation (1 with nothing owned, 0.75 with the
  item); `Game._computeDuelTempoScale`'s 1 / 0.75 / 0.45 (Largo+item
  combined) cases via a temporary synthetic player swapped into
  `Game._state.player` (no real run exists yet at that point in the file's
  test order) and restored afterward so the probe leaves no state behind
  for whatever runs next in the same shared jsdom window.
- `npm run test:react-duel-loss` (real browser, built output): ALL CHECKS
  PASSED. Extended with a new section between the pre-existing Largo
  click-check and Phase 0: grants Ritardando via the same `page.evaluate`
  convention Phase 1.5's Second Wind check already established (no shop UI
  offers a specific item yet), then RE-ENTERS the same boss fight for real
  (Ritardando only reads at fight start, so retroactively expecting an
  already-running sequencer to pick it up would test a behavior this item
  was never built to have) and confirms the freshly-started real
  sequencer's `getTempoScale()` reads 0.75, then a real Largo button click
  on top confirms 0.6*0.75=0.45(ish, float) on the SAME live sequencer --
  **the one behavior no jsdom test can reach**, `Game.setLargoEnabled`'s
  live-sequencer branch. Strips the item and re-enters once more afterward
  so the file's pre-existing phases (non-fatal block loss, i-frames,
  Second Wind revival, fatal defeat) run at the normal pace they were
  written against -- all of which still passed unchanged, confirming the
  extra re-entry cycles didn't disturb anything downstream.
- `npm run build`: clean, 51 modules (unchanged count -- both items live in
  the existing `items.js`, no new file).
- `npm run test:mobile`, `test:qa`, `test:react-qa`, `test:react-build`,
  ...
  PASSED, unaffected -- neither item touches any UI/CSS (both are
  invisible passive effects, like most of this item pool).
  `test:duel-balance`'s pre-existing early/regular/weak stalemate flag is
  unchanged, exit code 0 -- that sim doesn't model items at all.
- `npx vitest run`: 172/172, unchanged count -- neither item touches a
  `src/components/*.jsx` file, so no new React coverage was needed and
  none regressed.
- `npm run build:itch` + `npm run test:itch-build`: ALL CHECKS PASSED, no
  manifest change needed (`items.js` was already listed).

Version NOT bumped -- ITEMS is a large multi-run ticket (Jaxon's four +
round out to 8-12), and this run landed 2 of 4 signature items plus none of
the batch expansion; the "bump minor per completed feature" convention
applies once the ticket's box is actually checked.

**Not done, honest gaps:** FORTISSIMO (rack-capacity halving + double-size
tile rendering + an explicit Rewrite-interaction decision) and THE INVERTED
SCORE (flip-mapping + reversed-letter-order dictionary check) are both real,
separately-scoped remaining signature items. The AMENDED batch (health
items + 4-8 duel-gauge-space items) is fully untouched. ITEMS stays
unchecked.

**Genuinely-Jaxon-only, flagged rather than blocking:** Ritardando's 0.75
tempo scale and Poetic License's rarity ('rare')/shopPrice (40) -- round,
mutually distinguishable numbers, not balance-sim-tuned.

**Next:** FORTISSIMO is likely the more self-contained of the two remaining
signature items (rack-capacity/rendering work, no new validity-engine
change needed) -- THE INVERTED SCORE needs a real flip-mapping/reversed-
order dictionary check, a bigger lift. Once all 4 (or a documented subset
...
## 2026-08-22T11:59Z -- ITEMS ticket: THE INVERTED SCORE (4th of Jaxon's 4 signature items)

Picked up ITEMS at the exact point the prior run's own "Next" note left it:
THE INVERTED SCORE, the last of Jaxon's 4 signature items ("flips all tiles
upside-down; a word is playable ONLY if it reads as a real word
upside-down"). All 4 signature items now exist; the ticket's AMENDED batch
(health items + 4-8 duel-gauge-space items) is the only ITEMS scope still
open, so the box stays unchecked. Full reasoning is in GOALS.md's own
ORCHESTRATOR NOTE under the ITEMS ticket -- this entry summarizes.

**What landed** (`js/wordbound/items.js`, `js/wordbound/combat.js`,
`js/wordbound/lexicon.js`, `js/wordbound/game.js` -- no new file, no
manifest change needed):
- `FLIP_MAP`: the ticket's own conservative mapping (u<->n, m<->w, b<->q,
  d<->p, o/s/x/z/i self-flip). Every other letter has no entry.
- `Items.flipUpsideDown(word)`: maps each letter, then REVERSES the whole
  result (a physical tile strip turned 180 degrees does both at once).
  Self-checked against the classic real-world examples: SWIMS flips to
  SWIMS itself (a genuine upside-down palindrome), MOM flips to WOW.
  Returns null the instant any letter has no FLIP_MAP entry.
- `Items.upsideDownValid`/`hasInvertedScore`, and a new centralizing
  `Items.isWordValid(word, player)` that `combat.js`'s `playWord`/
  `previewWord` BOTH now call instead of each duplicating its own
  `isValidWord || bypassesWordValidity` OR chain -- a small refactor, not
  a behavior change for any existing case (confirmed by the full
  pre-existing suite staying green unchanged).

**Judgment call 1 (documented, not silently decided):** the ticket's
"playable ONLY if it reads as a real word upside down" reads as EXCLUSIVE.
While this item is owned, `isWordValid` REPLACES the whole normal validity
chain (plain dictionary check AND Poetic License's bypass), not an OR
alongside them -- so a completely ordinary word the player could always
play (e.g. "MOOD", real, but its flip "POOW" isn't) becomes UNplayable the
instant this item is picked up. Confirmed as the intended severity by the
ticket's own explicit "letters without a clean flipped form make a word
unplayable" callout, not an accident of implementation.

**Judgment call 2:** the ticket specifies the restriction but names no
compensating benefit, unlike the other 3 signature items (each pairs a
restriction with a real payoff). A pure restriction with zero upside would
be a shop item nobody would ever buy. Added `statMods.scoreMultiplier: 2.5`
(composes multiplicatively with Fortissimo's own 2x) so it's a real,
extreme trade rather than a dead purchase -- this run's own addition, not
dictated by the ticket text. Also set price 60g (above the other three's
40-50g, "cost/rarity accordingly" per the ticket) and the flavor reading of
"score" as sheet music, not a point total (the musical-pun conceit: the
music itself turns upside-down). All flagged for Jaxon.

**A real, non-hypothetical softlock risk found and closed:** the
pre-existing anti-softlock safety net (`ensureRackIsPlayable`/
`Lexicon.hasPlayableWord`) checks PLAIN dictionary validity -- with this
item owned it would call a rack "playable" because it contains a normal
word the player could never submit, letting a genuine permanent dead end
through. A node-script simulation against the real `LETTER_POOL` weights
(not guessed) found a fresh 7-tile rack fails the real flip-aware
playability check ~25% of the time on the first draw (same order as the
pre-existing Scribe-vowel-poor case this net was already sized for, so the
existing 5-retry limit is still enough there, <0.1% residual). But a
player who ALSO owns Fortissimo (rack capacity 7->4) sees a ~59% first-draw
fail rate, and 5 retries alone leave a real ~5% chance of a permanently
stuck fight for that specific two-rare-item combo -- simulated directly.
Fixed with `Lexicon.hasPlayableInvertedWord(rack)` (reuses the existing
anagram-key-set machinery, mapping each subset through FLIP_MAP first; a
short equivalence proof in its own comment covers why the reversal step
doesn't need separate handling) plus a higher retry ceiling used only when
this item is owned (`UNPLAYABLE_RACK_RETRY_LIMIT_INVERTED = 25`), bringing
the Fortissimo-combo residual risk down near 0.02% -- not claimed
impossible, brought back in line with this safety net's own pre-existing
"in practice enough" standard.

**Verified:**
- `npm test` (jsdom dom-check): ALL CHECKS PASSED, +27 new checks -- flip
  self-checks, 3 real gameplay scenarios found via a one-off node script
  against the actual bundled wordlist (MOOD real-but-unplayable-with-item,
  UOM not-real-but-playable-with-item scoring the predicted 13 damage via
  the real formula, CAT real-but-unplayable-with-item proving the replace-
  not-OR behavior), getScoreMultiplier composing with Fortissimo, 4
  hasPlayableInvertedWord checks (including one proving it genuinely
  disagrees with the normal check on the same rack), a seeded 300-sample
  shop-appearance check, and a real end-to-end fight via `Game.submitWord`
  (reject with no state mutated, then a real kill).
- `npx vitest run`: 174/174, unaffected (no `.jsx` touched -- combat.js is
  the shared choke point both apps already use, no new UI needed).
- `npm run build`: clean, 51 modules (unchanged).
- `npm run test:mobile`, `test:qa`, `test:react-qa`, `test:react-build`,
  `test:react-duel-loss`, `test:music-engine`, `test:branching-map`,
  `test:run-header`, `test:audio`, `test:drag-interrupt`: ALL CHECKS
  PASSED. (First attempt at `test:audio`/`test:itch-build`/
  `test:react-build` hit `EADDRINUSE`/a Playwright timeout from running
  several fixed-port Playwright scripts concurrently in this container --
  a self-inflicted resource/port collision from MY OWN parallel test runs,
  not a regression; re-ran each alone afterward and all passed clean.
  Noting this explicitly so a future run doesn't mistake concurrent local
  script contention for a real failure, and doesn't run these particular
  scripts in parallel with each other.)
- `npm run build:itch` + `test:itch-build`: ALL CHECKS PASSED, no manifest
  change needed.
- `npm run test:duel-balance`: same pre-existing stalemate flag every
  prior run documents, exit 0, unrelated (no item modeling).

Version NOT bumped -- all 4 signature items exist now, but the AMENDED
batch (health items + duel-gauge-space items) is still fully open; the
"bump minor" convention applies once that lands and the box is checked.

**Genuinely-Jaxon-only, flagged rather than blocking:** the 2.5x
compensating multiplier, the 60g price, filling in "no stated benefit,"
and the exclusive/replacing reading of "playable ONLY if" -- worth Jaxon's
explicit read given how significant "replaces validity entirely" is for a
shop item.

**Not done, honest gaps:** the AMENDED batch (health items + 4-8
duel-gauge-space items, 8-12 total per the ticket) is the only remaining
ITEMS scope. ITEMS ticket stays unchecked.

**Next:** the health items + duel-gauge-space batch is the natural next
chunk -- once it lands and the box is checked, return to SHAKESPEARE GUIDE
+ AUTHOR SHOPKEEPERS to wire the six author exclusives on top (its own
coordination note; the exclusives pool can reasonably draw from items that
...
## 2026-08-22T13:04Z -- SHAKESPEARE GUIDE + AUTHOR SHOPKEEPERS: exclusive items (closing this ticket)

Read GOALS.md in full, then PROGRESS.md's tail and ROADMAP.md per the
routine's own instructions. First unchecked queue item was SHAKESPEARE
GUIDE + AUTHOR SHOPKEEPERS, whose only remaining scope (per its own chain
of ORCHESTRATOR NOTEs) was step 2's exclusive-items half, explicitly
waiting on the ITEMS ticket -- which the prior run just finished. Landed
that remaining scope this run and checked the ticket's box for real.

**What landed** (one exclusive item/consumable per author, the ticket's
own "1-2," floor of the range -- see judgment call below):
- **The Ingenious Gentleman's Ledger** (Cervantes, rare item): `onWordPlayed`
  percent bonus that scales per letter past length 6 (nothing at 6, +10% at
  7, +20% at 8) -- layered on top of Lexicon.scoreWord's own flat
  lengthBonus rather than duplicating it, per THEME.md's own "past the
  usual length-bonus curve" wording.
- **An Ideal Word** (Wilde, uncommon item): flat `(5-length)*3` bonus at
  length <=4, nothing past it -- the deliberate opposite bracket from
  Cervantes's item, per THEME.md's own framing of the two as a matched
  pair.
- **A Truth Universally Acknowledged** (Austen, uncommon item): +10% on
  any NON-repeat play, reading the exact `ctx.result.isRepeat` field
  combat.js already sets (the same field Encore's own hook already reads)
  -- "codifies the repeat penalty into a bonus" per THEME.md's own words,
  no new engine surface needed.
- **The Tell-Tale Meter** (Poe, rare item): `onWordPlayed` heals 10% of
  the word's damage as ink, capped at maxInk -- a genuinely PROPORTIONAL
  Vampiric-style heal, a new pattern distinct from the existing flat-
  per-tile Vampiric TILE VARIANT already in game.js.
- **A Certain Slant of Ink** (Dickinson, uncommon item): -1 to BOTH
  Overcharge and Rewrite ink costs, floored at 1 so neither can go free.
- **The Wine-Dark Litany** (Homer, CONSUMABLE -- lives in consumables.js,
  not items.js, per THEME.md's own wording): reuses Index Card Shard's
  exact `bonusDamageUntilEndOfTurn` mechanism (+10 instead of +15) rather
  than inventing a second one.

**The exclusivity mechanism -- genuinely new machinery, verified as a real
gate, not just code inspection:** a new `exclusiveTo: <authorId>` field.
`game.js`'s `rollShopOptions` filters both the item AND consumable pools
by it (checked AFTER `rollShopkeeper()` has already rolled the visit's
author, per that function's own pre-existing call-order comment) --
deterministic, not a probability weight. Since Treasure/boss-reward rolls
and the random enemy-drop roll have NO shopkeeper context at all,
`rollTreasureOptions`/`rollBossRewardOptions` (game.js) and
`Consumables.rollConsumableDrop` now exclude every `exclusiveTo` item/
consumable UNCONDITIONALLY -- closing three side doors that would
otherwise have let an "exclusive" leak out through Treasure nodes, boss
kills, or ordinary monster drops. Added `Game._rollTreasureOptions`/
`_rollBossRewardOptions` test-only hooks (mirroring the pre-existing
`_rollShopOptions`) so this could be verified directly rather than by
inspection.
...
rewriteRack, renderInkSpendButtons) and `CombatScreen.jsx` (cost checks +
both button labels). Every one now reads through two new getters,
`Items.getOverchargeInkCost(player)`/`getRewriteInkCost(player)` (sum
owned `overchargeCostReduction`/`rewriteCostReduction` statMods, floor of
1 ink), so the discount is honored everywhere the cost is charged,
checked, or displayed -- not drifting apart across call sites. Confirmed a
true no-op without the item: every PRE-EXISTING ink-spend test in
test/dom-check.js (which asserts against the raw Combat constants
directly) passed unchanged.

**Judgment call on scope, documented rather than silently decided:** the
ticket asks for "1-2 EXCLUSIVE ITEMS" per author -- landed exactly 1 (the
floor), the same "depth of verification over item count for a single
bounded run" reasoning the ITEMS ticket's own AMENDED-batch note already
used for its 4-of-8 duel-gauge items. Each author's OTHER concept from
THEME.md's table (Homer's Rhapsode's Girdle -- THEME.md's own cell already
says this one is flavor text for the existing guaranteed-consumable-slot
logic, not a separate mechanic; Cervantes's Rocinante's Last Furlong;
Austen's Persuasion's Turn; Dickinson's I Dwell in Possibility; Poe's
Quoth; Wilde's A Portrait in the Attic) remains a real, spec'd-in-the-
bible, unbuilt concept -- additive future expansion, not a broken promise.
Quoth (a one-time repeat-penalty immunity) was deliberately passed over
for Poe's OTHER concept specifically because it would have needed a new
option threaded through `Combat.playWord`'s `isRepeat` detection across
both the turn-based and duel combat paths -- a real, separately-scoped
engine change, unlike the other five which each mapped onto existing
machinery.

**Verified:**
- `npm test` (jsdom dom-check): ALL CHECKS PASSED, +33 new checks -- for
  each of the 6 exclusives: appears in its own author's shop across up to
  200 seeded samples, and is confirmed ABSENT from a different author's
  shop across 100 seeded samples (deterministic exclusion -- a single hit
  anywhere would have failed this); none of the 6 appear with no
  shopkeeper set (100 samples), in Treasure options (100 samples), or in
  boss-reward options (100 samples -- the check that actually exercises
  the new rollBossRewardOptions filter, since 2 of the 5 items are rare
  and would otherwise pass its pre-existing rarity gate); each item's own
  mechanical effect in isolation (Ledger's 3 length brackets, An Ideal
  Word's 3, Truth's repeat/non-repeat pair, Tell-Tale Meter's heal +
  overheal-cap + zero-damage no-op, Certain Slant of Ink's getters
  including an extreme-fake-item floor-at-1 test mirroring Sordino's own
  0.9-clamp convention, Wine-Dark Litany's consumable effect + a
  200-sample confirmation it never surfaces from the random enemy-drop
  roll). Every pre-existing ink-spend test passed unchanged.
- `npx vitest run`: 182/182 (up from 181) -- 1 new CombatScreen test
  driving A Certain Slant of Ink through the REAL component (button labels
  show the reduced cost, a real Rewrite click charges the reduced amount),
  not just the getter in isolation.
- `npm run build`: clean, 51 modules (unchanged -- no new file).
- `npm run test:mobile`: ALL CHECKS PASSED, unaffected (no CSS/layout
  touched -- every new item is invisible/passive or changes button TEXT
  only, already covered by the pre-existing ink-spend mobile section).
- `npm run test:qa`, `test:react-qa`, `test:react-build`,
  `test:react-duel-loss`, `test:music-engine`, `test:branching-map`,
  `test:run-header`, `test:audio`, `test:drag-interrupt`: ALL CHECKS
  PASSED, unaffected.
...
- `npm run test:duel-balance`: same pre-existing early/regular/weak
  stalemate flag every prior run's own note documents, exit code 0,
  unrelated (this sim doesn't model items or shops).

Version bumped v0.6 -> v0.7 (`wordbound.html`, `MainMenu.jsx`, its Vitest
expectation) -- SHAKESPEARE GUIDE + AUTHOR SHOPKEEPERS is now genuinely
finished (guide intro, all 6 quirks, all 6 portraits, all 6 exclusives)
and checked off in GOALS.md.

**Genuinely-Jaxon-only, flagged rather than blocking:** every new numeric
value (Ledger's 10%/letter, An Ideal Word's `(5-len)*3`, Truth's flat 10%,
Tell-Tale Meter's 10% heal rate, Certain Slant of Ink's -1/-1) is this
run's own tuning call, not balance-sim-verified (same pre-existing
`test/balance-simulation.js` AudioContext-crash gap every item-adding run
in this file already flags). Also flagged: the "pick 1 of 2, mechanically-
closest-concept-wins" selection method, and whether the 5 deferred
concepts are worth a future pass.

**Not done, honest gaps:** the 5 deferred exclusive-item concepts listed
above. No shop has yet handed any of these 6 new items/consumable to a
real player in an actual run (same "reachable through the existing roll
pools, will surface naturally" gap every prior item batch has had at
landing time).

**Concurrent-push discovery, resolved before pushing (not a collision on
the SAME work, unlike this repo's earlier ones):** at push time, `origin/
main` had moved one commit past this run's own starting point (9949859) --
Jaxon himself pushed `bac399f`, adding a new **DEPLOY** ticket (public play
URL via GitHub Pages) to GOALS.md, explicitly queued FIRST ("do this
FIRST, it unblocks his playtest of everything already built"). Rebased
this run's commit onto it (a one-line textual conflict in GOALS.md, both
patches touching the same insertion point -- resolved by keeping both:
Jaxon's new ticket, then this run's `[x]` on SHAKESPEARE GUIDE) and pushed
as a clean fast-forward. No code/content conflict at all -- Jaxon's commit
only touched GOALS.md.

**Next:** DEPLOY is now genuinely the first unchecked queue item, and per
Jaxon's own explicit instruction ("do this FIRST") takes priority over
...
## 2026-08-22T14:50Z -- REGULAR ENEMIES: dom-check.js real-floor-RNG audit
## (real remaining scope (1)) done

**What this closes:** the previous two runs' own demonstrated blocker --
wiring a `.piece` onto a real weak-tier `MONSTER_DEFS` entry makes it
reachable via real floor-generation RNG, and any `test/dom-check.js` combat-
entry block that doesn't route to a known non-`.piece` def crashes hard
(`initAudioContext()` throws -- no `window.AudioContext` in jsdom). This
ticket's own prior note named exactly 2 blocks (the Volatile-tile next-fight
reset, the MAGNIFICENT-bonus-gold check) as confirmed examples and called
the audit "small, self-contained." It was bigger than that framing: the
audit found the file's own FIRST-EVER combat entry (New Run -> character
select -> the first node-pill click) is the single biggest exposure --
`Game.startRun`'s `Floor.generateBranchingFloor` picks EVERY row-0 start
node's regular defId (2-3 lanes, all real RNG) before any test code runs at
all, gating the entire rest of the 4500-line file -- plus a NEW class of
hazard the prior note didn't name: 5 blocks that pick
`Object.keys(MONSTER_DEFS)[0]` (fragile, relies on insertion order) and 5
MORE blocks that hardcode a literal regular defId ('slime' x4, 'sentinel'
x1) which LOOK pinned-safe but aren't -- nothing stops 'slime' or 'sentinel'
specifically from being the first regular a future run wires a `.piece`
onto.

**Confirmed the hazard is real, not theorized, before fixing anything:**
temporarily wired `.piece: Pieces.morningMood, pushesToDefeat: 1` onto
'slime' (weak) directly in `js/wordbound/monsters.js` (never committed) and
ran `npm test` 4 times pre-fix: 4/4 crashed with the exact
`SCRIPT CRASHED: TypeError: (window.AudioContext || window.webkitAudioContext)
is not a constructor` a prior run's own note already documented -- this
time at the 'slime'-hardcoded `killWith` helper (a different call site than
originally demonstrated, confirming this is a systemic pattern across the
file, not one fixable line). Repeated with 'sentinel' (strong/elite) to
confirm the elite path has the identical exposure.

**Fix:** two small shared helpers added at the top of `test/dom-check.js`:
`firstSafeDefId(defs, tier)` (first def without `.piece`, optionally
tier-filtered) and `pinNodeAwayFromDuelMode(node, Monsters)` (rewrites a
combat/elite node's `defId` to a safe same-tier alternative ONLY if its
current def actually carries `.piece` -- a true no-op against today's real
`MONSTER_DEFS`, since none do yet). Applied at every real exposure point:
(1) right after character-select, before the file's first-ever node-pill
click, pins EVERY node in the freshly-generated `state.floor.nodes` (all
branching-map start lanes, not just one) -- since this same array is never
regenerated for the rest of the file (the two spots that call
`Game._advanceFloor()` for a real check save/restore `state.floor` around
it), this single early pin transitively covers the two "next uncleared
node" blocks too, but (2) defense-in-depth pins were added directly in
those two named blocks anyway, since the ticket's own audit called them out
by name; (3) all 5 `Object.keys(MONSTER_DEFS)[0]` picks replaced with
`firstSafeDefId(MONSTER_DEFS)` (identical result today); (4) all 5
hardcoded-literal node objects ('slime' x4, 'sentinel' x1) now call
`pinNodeAwayFromDuelMode` on the constructed node right after creation.
Left the boss side untouched -- `'boss_unabridged'` picked deliberately
everywhere as the one non-`.piece` boss is already the correct, intentional
analog (a fixed per-boss fact across only 4 bosses total, not a "next
content" moving target the way regulars are), not a gap this run needed to
touch.

**VERIFIED, this run:**
- Re-ran the SAME temporary `.piece` injection (both 'slime' and
  'sentinel', each in a separate isolated pass) WITH the fix in place:
  `npm test` clean 5/5 with 'slime' duel-ified, clean 3/3 with 'sentinel'
  duel-ified -- both previously 4/4 (spot-checked) reproducible crashes,
  now genuinely tolerated.
- Reverted the injection; confirmed `git diff js/wordbound/monsters.js`
  came back completely empty afterward -- nothing shipped to game content
  this run, this is test-harness-only.
- Against the REAL, unmodified `monsters.js` (today's actual state, zero
  `.piece` regulars): `npm test` clean on 2 separate runs; 1 separate run
  hit the SAME pre-existing "STOLEN LETTERS boss-kill ends in GAME_OVER
  instead of TILE_REWARD" flake this file's own history already documents
  multiple times under different names ("audio: dying to a counterattack",
  the earlier "STOLEN LETTERS flake" note). Isolated via `git stash` back to
  the unmodified base: reran 5 times with none of this run's changes
  present, all 5 clean -- consistent with this being real, already-
  documented pre-existing flakiness recurring at its known rate, not
  something this run's change caused. Additional confirmation: with zero
  `.piece` regulars anywhere, `firstSafeDefId(MONSTER_DEFS)` returns the
  IDENTICAL first key `Object.keys(MONSTER_DEFS)[0]` already returned --
  this run's change is a byte-for-byte behavioral no-op against today's
  real game state, so it cannot be the cause of a new flake.
- `npm run test:react`: 183/183, unaffected (no `src/` file touched).
- `npm run build`: clean, unaffected (44 modules).
- `node --check test/dom-check.js`: clean.

**Not done, honest gaps:** this closes real remaining scope (1) in full --
broader than the prior note's own framing, since the literal-defId and
`Object.keys()[0]` hazards weren't named there but are the same class of
bug and are now fixed too. Real remaining scope (2) (wire the 3 landed
early-tier pieces into real weak-tier `MONSTER_DEFS` entries +
`retiredFromPool` + `floor.js`'s pool filter) and (3) (compose the 6
mid/late regulars) are still untouched -- this run was scoped to the audit
alone, per its own "small, self-contained" framing. Version NOT bumped --
nothing shipped to real gameplay this run. This is a test-only change, so
the LIVE DEPLOY refresh rule does not apply (no game code/assets changed).

**Genuinely-Jaxon-only:** none this run -- pure test-infrastructure hardening.

**Next:** real remaining scope (2) is now genuinely unblocked -- wiring a
`.piece` onto a real weak-tier `MONSTER_DEFS` entry (the exact diff a
concurrent run's own history already has, per the prior note) should no
longer crash `npm test`, since every combat-entry block in
`test/dom-check.js` now tolerates it. A future run should still run the
FULL verification suite after that wiring lands rather than just trust this
audit -- this run's confirmation used a synthetic/reverted injection on
...
## 2026-08-22T15:58Z -- REGULAR ENEMIES: 3 early-tier regulars now real, reachable weak-tier monsters (real remaining scope (2) done)

Picked up REGULAR ENEMIES' real remaining scope (2), the next chunk every recent
note has been pointing at: wire the 3 already-composed early-tier pieces
(Gymnopédie No. 1, Air on the G String, Morning Mood) into real `MONSTER_DEFS`
entries instead of leaving them landed-but-inert. Both prior blockers (the
dom-check.js real-floor-RNG audit and its React/Vitest equivalent) were already
fixed by earlier runs, so this was genuinely unblocked going in.

**What landed:**
- `js/wordbound/monsters.js`: 3 new weak-tier defs -- `gymnopediste` (The
  Gymnopédiste, 🩰), `gstring` (The G String, 🎻), `morningmood` (Morning
  Mood, 🌅) -- each carrying a real `piece`/`pushesToDefeat: 1` (the same
  `.piece` auto-detection cutover every boss already uses), real weak-tier
  HP/attack/goldDrop numbers, and a real `traitPhases` weakness (kept real,
  not dead data, because the monster-info panel's "Weakness:" line
  unconditionally reads it even in duel mode and would crash on an empty
  array).
- Retired the 4 old generic weak-tier defs (slime/gremlin/wisp/glossary) from
  the real floor-generation draw pool via a new `retiredFromPool: true` flag
  on each -- the defs themselves are untouched (several tests construct them
  directly and still do), only `floor.js`'s `pickCombatDefId` now filters
  them out of a fresh floor's RNG pick.
- **Real bug found and fixed, not just new content:** a prior run's own
  "inert glyph groundwork" note assumed `Monsters.createMonster`/
  `createBoss` already carried a def's `.glyph` field through onto the live
  monster instance -- they didn't. `renderCombat` (game.js) and
  `CombatScreen.jsx` both read `m.glyph`/`monster.glyph` off the instance,
  not the def, so every def's `.glyph` was silently dropped and would never
  have rendered, no matter which def eventually set one. Fixed both factory
  functions to copy `glyph: def.glyph` through, confirmed end-to-end (see
  verification below) rather than assumed fixed.
- Version bumped v0.7 -> v0.8 (`MainMenu.jsx`/`wordbound.html`/
  `MainMenu.test.jsx`) -- real gameplay content shipped.

**Two pre-existing tests broke for real, both fixed (not silently
worked around):**
- `src/test/duelIntegration.test.js`'s "ends the run on player-defeated"
  test used a fixed seed (`'duel-start-4'`) whose floor's row-0 start lanes
  BOTH now roll a duel-mode weak regular -- exactly the scenario
  `findAvailableCombatNodeId`'s own prior-run safety net was built to catch,
  now genuinely tripped for the first time (it correctly threw instead of
  crashing on `initAudioContext()`). Retargeted to seed `'duel-start-4-safe'`,
  confirmed to still have a non-duel start node.
- Same file's "a monster def with .piece starts a real duel fight" test
  searched 40 seeds for a real floor draw landing on `'slime'` to monkey-patch
  `.piece` onto -- `'slime'` is no longer real-floor-drawable now that it's
  retired from the pool, so no seed in the range would ever match anymore.
  Retargeted onto `'serpent'` (normal-tier, still pool-drawable on floor 1),
  same monkey-patch approach.

**VERIFIED, this run -- the ticket's own full VERIFY bar, run for real for
the first time since it stopped being hypothetical:**
- `npm test` (dom-check.js): ALL CHECKS PASSED, 2 runs. Unaffected by this
  run's change -- the file's own hardcoded literal-defId checks and
  `pinNodeAwayFromDuelMode` calls construct nodes directly rather than
  drawing from the real RNG pool, so `retiredFromPool` doesn't touch them.
- `npm run test:react` (Vitest): 183/183 clean after both test fixes above,
  2 reruns.
- `npm run build`: clean, 54 modules.
- `npm run test:qa` (real Chromium, vanilla `wordbound.html`): ALL CHECKS
  PASSED, including the organic first-combat phase.
- `npm run test:react-qa` (real Chromium, built React app): ALL CHECKS
  PASSED, same coverage.
- `npm run test:mobile`: ALL CHECKS PASSED, 375/414px, unaffected (no CSS
  touched this run).
- `npm run test:branching-map`: ALL CHECKS PASSED, 180 floors/seeds swept --
  no orphan/unreachable-node regressions from the pool filter.
- `npm run test:duel-balance` (the ticket's own "virtual-clock sim confirming
  the tier curve" VERIFY line): early/regular/weak still reads win 100% /
  loss 0%, no SAFETY flags. The sim reads Morning Mood's piece data directly
  rather than through `MONSTER_DEFS`, so the numbers are unchanged by this
  run, but this is the first time that VERIFY line is meaningful against a
  REAL wired-in weak-tier duel monster rather than the sim's own standalone
  reference.
- `npm run test:react-duel-loss`, `test:music-engine`, `test:audio`,
  `test:drag-interrupt`, `test:run-header`, `test:react-build`: ALL CHECKS
  PASSED, unaffected.
- `npm run build:itch` + `npm run test:itch-build`: ALL CHECKS PASSED --
  confirms the wiring survives the itch bundle path too.
- **Direct per-species Playwright smoke** (an ad-hoc script, run then
  deleted -- a one-off verification pass, not a new permanent gate): force-
  entered each of the 3 new regulars individually in a real headless-Chromium
  run apiece (not just whichever def the organic QA run's RNG happened to
  draw). All 3: real duel mode confirmed (`state.monster.duel === true`,
  correct `defId`), the glyph fix confirmed literally rendered in the DOM
  (e.g. `"📄 🩰 The Gymnopédiste"` as `.monster-name`'s real textContent, not
  just that the field was set), each resolved to a real win (TILE_REWARD)
  within a few turns, zero console/page errors across all 3. Satisfies the
  ticket's own "Playwright duel smoke per tier (win path)" line for the
  early tier specifically, rather than relying on the organic run's single
  RNG-dependent sample.
- **Loss path, deliberately not separately smoke-tested:** the sim data
  above (0% loss across every bot profile at this tier) shows a genuine loss
  against these 3 pieces isn't really achievable by design at the early
  tier, matching the header's own "nearly safe" intent -- the DUEL LOSS
  MECHANIC itself already has real coverage (test:react-duel-loss, against a
  synthetic piece), so this isn't a gap, it's the early-tier design working
  as intended.

**Genuinely-Jaxon-only:** none this run.

**Not done, honest gaps:** real remaining scope (3) -- compose the 6 mid/
late regulars (Gnossienne/Invention/Metronome for 'normal' tier, Swarm/
Sabbath/Organist for 'strong' tier -- THEME.md's own table already names and
PD-vets all of them) and repeat this exact wiring pattern for each -- is
still fully unstarted. This ticket stays open; only 3 of 9 regulars are
real. `floor.js`'s pool filter is already generic across every tier, so no
further engine change should be needed for scope (3), just content + the
same retirement + a fresh full VERIFY pass.
...
you don't expect them, per THEME.md) composed, PD-vetted, and unit-tested
the same way this run's 3 pieces were, proven standalone before wiring, per
this ticket's own established precedent. Meaningfully more of this ticket's
scope remains than has landed (6 of 9 regulars, normal/strong-tier
